...
VegMem <- function(ClimVar, ClimVar2, Region, Cumlags, FromY, ToY){
  ...
  # PREPARE DF----
  export_df <- as.data.frame(matrix(rep(NA, 17), ncol = 17))
  colnames(export_df) <- c(names(ModelEval_ras), "Pixel", "Detrending")
  # MODELS----
  ...
  for(pixel in Data_Pos){ # loop non-NA pixels
    ...
    sink(file = paste(Dir.Memory, "/", Region, ".txt", sep=""))
    print(paste(pbi, length(Data_Pos), sep="_"))
    sink()
    T_Begin <- Sys.time() # note time when calculation is started (needed for estimation of remaining time)
    Detrends_ls <- list("detrend(NDVI_vecraw, tt = 'linear')",
         "NDVI_vecraw-ma(NDVI_vecraw, order = 12, centre = TRUE)",
         "NDVI_vecraw/ma(NDVI_vecraw, order = 12, centre = TRUE)")
    names(Detrends_ls) <- c("Linear", "Additive", "Multiplicative")
    NDVI_vecraw <- as.vector(NDVI_ras[pixel]) # eytract data
    for(Detrend_Iter in 1:3){
      ## DATA ----
      ### NDVI stuff -----
      NDVI_vecdet <- eval(parse(text = Detrends_ls[[Detrend_Iter]]))
      # create NDVI data frame
      NDVI_df <- data.frame(Month = rep(1:12, length(NDVI_vecraw)/12), NDVI_raw = NDVI_vecraw, NDVI_de = NDVI_vecdet)
      # NDVI_df <- na.omit(NDVI_df) # to deal with NA's introduced by new detrending methods at end and beginning of time-series
      ...
      ## WRITING INFORMATION TO DF----
      export_df[Counter,] <- c(as.numeric(c(AIC(Mod), c_NDVI, c_Clim, PCABest[1]-1, c_Clim2, PCABest[2]-1, Vars, ps)), pixel, names(Detrends_ls)[Detrend_Iter])
      Counter <- Counter + 1
    } # end of detrending loop
